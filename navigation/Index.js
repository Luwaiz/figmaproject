...
function LoggedOutScreen() {
  const [appIsLaunched, setAppIsLaunched] = React.useState(null);

  const GetItem = async () => {
    try {
      const value = await AsyncStorage.getItem("appIsLaunched");
      if (value !== null) {
        setAppIsLaunched(false);
      } else {
 ...
 React.useEffect(()=>{GetItem()},[])
  return (
    appIsLaunched !==null &&(
     ...
     {appIsLaunched===true && (<Stack.Screen name="OnBoard" component={OnBoard} />)} 
      ...
      <Tab.Screen
        ...
        options={{
          ...
          tabBarIcon: ({ focused }) => (
            <View
              style={[
                focused ? styles.activeIndicator : styles.inActiveIndicator,
              ]}
            >
              <Ionicons
                name="person-sharp"
                size={22}
                color={focused ? "#470440" : "white"}
              />
            </View>
          ),
        }}
